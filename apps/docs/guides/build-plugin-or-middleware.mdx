...
```ts
import { middleware, Redop } from "@useagents/redop";
...
new Redop({ name: "middleware-demo" })
...
```ts
import { definePlugin, Redop } from "@useagents/redop";
...
new Redop({ name: "plugin-demo" }).use(timingPlugin({}));
```
