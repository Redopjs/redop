...
Redop is Bun-first. `.listen()` starts a long-running Bun HTTP server, which fits Railway, Fly.io, Docker, and similar always-on hosts. Cloudflare, Vercel, and Node use portable fetch adapters on top of the same MCP handler.
...
| Unkey | Docker deployment with preview environments, rollbacks, and Sentinel | `http` + `.listen()` | none | Private beta |
...
- Adapter targets: export `toCloudflare(app)`, `toVercel(app)`, or `listenNode(app)`.
