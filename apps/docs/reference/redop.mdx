...
| `handler(opts?)` | Return a portable fetch handler for Cloudflare, Vercel, Node, and other runtimes. |
| `listen()` / `listen(3000)` / `listen(opts)` | Start the HTTP or stdio transport. Bun is required for HTTP `.listen()`. |
